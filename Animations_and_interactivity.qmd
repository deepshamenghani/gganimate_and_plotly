...
This blog was created using [Quarto](https://quarto.org/) and published with Github actions. 
:::
...
Note that the US population dataset was downloaded from the census website and cleaned for purposes of this session. Link to the raw data can be found at [census.gov](https://www.census.gov/data/tables/time-series/dec/popchange-data-text.html)
:::
...
I wanted to showcase relative change in population among all states on the US map between the period 1910 to 2020. The plot below showcass relative population among states in the year 2020.
...
## Plot for single year ----
usa_population_map_single_year <- plot_usa_population_map(data = population_dataset_lat_long %>% filter(Year %in% (1910))) + theme(legend.position = "none")
...
## Not always rainbows and butterflies with GGAnimate package
...
Now let's try the same example and recreate that animation with plotly.
